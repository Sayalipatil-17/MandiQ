    ...
    <div className="min-h-screen bg-[#f4f6f4] pb-20 max-w-md mx-auto">
      ...
      <div className="bg-gradient-to-br from-[#1e5631] via-[#2d6a3e] to-[#16a34a] px-6 pt-8 pb-6 rounded-b-[2rem] shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white">मंडी जानकारी</h2>
            <p className="text-white/70 text-xs">सभी फसलों के भाव</p>
          </div>
        </div>

        {/* Mandi selector */}
        <p className="text-white/70 text-xs mb-2">मंडी चुनें</p>
      ...
      <div className="px-5 py-4">
        {ld && (
          <div className="flex items-center gap-2 text-[#2d6a3e] py-8 justify-center">
            ...
            <span className="text-sm">भाव लोड हो रहे हैं…</span>
          </div>
        )}

        {!ld && (
          <>
            <p className="text-sm text-gray-500 mb-3">{selectedMandiObj.emoji} {selectedMandiObj.label} — आज के भाव</p>
                  ...
                  {c.price !== null ? (
                    <div className="text-right">
                      ...
                      <div className={`flex items-center gap-1 justify-end text-xs font-semibold mt-0.5 ${c.trend === 'up' ? 'text-green-600' : c.trend === 'down' ? 'text-red-500' : 'text-gray-400'}`}>
                        {c.trend === 'up' && <><TrendingUp className="w-3.5 h-3.5" /> +₹{c.change} बढ़ी</>}
                        {c.trend === 'down' && <><TrendingDown className="w-3.5 h-3.5" /> ₹{Math.abs(c.change)} घटी</>}
                        {c.trend === 'stable' && <><Minus className="w-3.5 h-3.5" /> कोई बदलाव नहीं</>}
                      </div>
                    </div>
                  ) : (
                    <span className="text-xs text-gray-400 bg-gray-50 px-3 py-1.5 rounded-xl">डेटा नहीं</span>
                  )}
                </div>
              ))}
            </div>
